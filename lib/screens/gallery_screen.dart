...
class _GalleryScreenState extends State<GalleryScreen> {
  ...
  Future<void> _refreshCollections() async {
    ...
    await context.read<PhotoProvider>().refreshCollections(service.fetchCollectionBlock);
  }
  ...
  void _onScroll() {
    ...
    if (_imageService != null && _photos.isNotEmpty) {
      ...
      final visibleRows = (_scrollCtrl.position.viewportDimension / itemHeight).ceil();
  ...
  Future<void> _loadMore({bool reset = false}) async {
    ...
    if (bids.isEmpty) {
      setState(() { _photos.clear(); _allBids = []; _hasMore = false; _loading = false; });
      return;
    }
    ...
    try {
      ...
      if (reset) {
        ...
        if (_allBids.isNotEmpty) {
          final blocks = await service.getLocalBlocks(bids: _allBids, page: 1, limit: 40);
          final photos = blocks.map(PhotoItem.fromBlock).toList();
          if (mounted) setState(() {
            _photos..clear()..addAll(photos);
            _page = 2;
            _hasMore = photos.length == 40;
            _loading = false;
          });
          // 预加载第一页图片到内存
          if (_imageService != null) _preloadImages(_imageService!, 0, photos.length);
        } else {          // 没有本地缓存，保持 loading 状态直到同步完成
      ...
      // 翻页：直接从本地缓存读
      final blocks = await service.getLocalBlocks(bids: _allBids, page: _page, limit: 40);
      final photos = blocks.map(PhotoItem.fromBlock).toList();
      if (mounted) setState(() {
  ...
  Future<void> _loadByTag({bool reset = false}) async {
    ...
    setState(() {
      _loading = true;
      _error = null;
      if (reset) { _photos.clear(); _page = 1; }
    });
    try {
      final service = BlockService(context.read<ConnectionProvider>());
  ...
  Future<List<String>> _loadCachedBids(List<String> collectionBids, {String? tag}) async {
    final prefs = await SharedPreferences.getInstance();
    final key = 'gallery_bids_${collectionBids.join(',')}${tag != null ? '_$tag' : ''}';
    return prefs.getStringList(key) ?? [];
  }

  Future<void> _saveCachedBids(List<String> collectionBids, List<String> bids, {String? tag}) async {
    final prefs = await SharedPreferences.getInstance();
    final key = 'gallery_bids_${collectionBids.join(',')}${tag != null ? '_$tag' : ''}';
    await prefs.setStringList(key, bids);
  }

  Future<void> _syncInBackground(List<String> collectionBids, {String? tag}) async {
    if (_syncing) return;
    setState(() { _syncing = true; _syncFetched = 0; _syncTotal = 0; });
    try {
      final service = BlockService(context.read<ConnectionProvider>());
      final newBids = await service.syncCollectionBids(
        ...
        onProgress: (fetched, total) {
          if (mounted) setState(() { _syncFetched = fetched; _syncTotal = total; });
        },
      );
      ...
      if (mounted) setState(() { _syncing = false; _loading = false; });
    }
  }

  Future<void> _preloadImages(ImageService imageService, int fromIndex, int count) async {
    final end = (fromIndex + count).clamp(0, _photos.length);
    for (var i = fromIndex; i < end; i++) {
      final photo = _photos[i];
      if (ImageCacheHelper.getMemoryImage(photo.cid, variant: ImageVariant.squareThumb) != null) continue;
      final file = await ImageCacheHelper.getCachedImage(photo.cid, variant: ImageVariant.squareThumb);
      if (file != null) {
        final bytes = await file.readAsBytes();
        ImageCacheHelper.cacheMemoryImage(photo.cid, bytes, variant: ImageVariant.squareThumb);
      }
    }
  }

  Future<void> _refresh() async {
    setState(() { _page = 1; _hasMore = true; _error = null; });
        ...
        title: Row(
          children: [
            Container(
              ...
              child: const Icon(Icons.photo_library_rounded, color: Colors.white, size: 16),
            ),
  ...
  /// 在 body 上叠加全屏手势检测层
  Widget _buildBodyWithEdgeGesture(ImageService imageService, bool hasCollections) {
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              child: Icon(Icons.cloud_off_rounded, size: 40, color: cs.onSurfaceVariant),
            ),
            ...
            FilledButton.icon(
              onPressed: () => Navigator.push(
                ...
                if (context.read<ConnectionProvider>().hasActiveConnection) _loadMore();
              }),
  ...
  Widget _buildGrid(ImageService imageService) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        const spacing = 2.0;
        final crossAxisCount = ((constraints.maxWidth + spacing) / (minItemWidth + spacing))
            .floor()
            .clamp(3, 8);

        return GridView.builder(
          key: ValueKey('${_selectedCollectionIndex}_${_selectedTag}'),
          ...
          itemBuilder: (context, index) {
            ...
            final itemWidth = (constraints.maxWidth - spacing * (crossAxisCount - 1) - 4) / crossAxisCount;
            return PhotoGridItem(
              ...
              onTap: () async {
                ...
                if (result != null && _scrollCtrl.hasClients) {
                  ...
                  final itemTop = row * (itemWidth + spacing) + 2;
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              child: Icon(Icons.wifi_off_rounded, size: 36, color: cs.onErrorContainer),
            ),
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              height: 80,
              decoration: BoxDecoration(color: cs.surfaceContainerHigh, shape: BoxShape.circle),
              child: Icon(Icons.photo_library_outlined, size: 40, color: cs.onSurfaceVariant),
            ),
            const SizedBox(height: 24),
            Text('暂无图片',
                style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w600)),
...
class _CollectionDrawerState extends State<_CollectionDrawer> {
  ...
  bool _isExpanded(PhotoCollection col) {
    // 默认：选中的集合自动展开
    return _expanded[col.bid] ?? (widget.selectedIndex != null &&
        ...
        child: Column(
          ...
          children: [
            Padding(
              ...
              child: Row(
                children: [
                  Icon(Icons.collections_bookmark_rounded, color: cs.primary, size: 20),
                  const SizedBox(width: 10),
                  Text(
                    '我的集合',
            ...
            Expanded(
              child: ListView.builder(
                ...
                itemBuilder: (context, i) {                  final col = widget.collections[i];
            ...
            InkWell(
              ...
              child: Padding(
                ...
                child: Row(
                  children: [
                    Icon(Icons.settings_rounded, size: 20, color: cs.onSurfaceVariant),
          ...
          child: Column(
            ...
            children: [
              ...
              ListTile(
                ...
                leading: Icon(
                  col.isDefault ? Icons.star_rounded : Icons.star_outline_rounded,
...
class _TagTile extends StatelessWidget {
  const _TagTile({required this.tag, required this.selected, required this.onTap});
  final String tag;
  final bool selected;
      ...
      child: Material(
        ...
        child: InkWell(
          ...
          child: Padding(
            ...
            child: Row(
              children: [
                Icon(Icons.label_outline_rounded, size: 15,
                ...
                if (selected)
                  Icon(Icons.check_rounded, size: 14, color: cs.onSecondaryContainer),
              ],
            ),
          ),
        ),
            ...
            child: Row(
              children: [
                ...
                if (selected && trailingWidget == null)
                  Icon(Icons.check_rounded, size: 16, color: cs.onPrimaryContainer),
              ],
            ),
          ),
        ),
        ...
        child: Column(
          ...
          children: [
            ...
            TextFormField(
              ...
              autofocus: true,
              validator: (v) => v == null || v.trim().isEmpty ? '请输入 BID' : null,
            ),
            if (_error != null)
              Padding(
          ...
          onPressed: _submitting
              ...
              : () async {
                  ...
                  try {
                    ...
                    setState(() { _error = '加载失败：$e'; _submitting = false; });
                  }
                },
          child: _submitting
              ? const SizedBox(
